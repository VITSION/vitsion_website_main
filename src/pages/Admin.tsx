...
const processFile = async (file: File): Promise<File> => {
    if (file.type === 'image/heic' || file.name.toLowerCase().endsWith('.heic')) {
        try {
            ...
            return new File([blob], file.name.replace(/\.heic$/i, '.webp'), { type: 'image/webp' });
        } catch (e) {
...
const Admin = () => {
    ...
    const onCropComplete = (_croppedArea: { x: number; y: number; width: number; height: number }, croppedAreaPixels: any) => {
    ...
    const handleCropSave = async () => {
        ...
        try {
            ...
            const file = new File([croppedImageBlob], "cropped_image.webp", { type: "image/webp" });
            const url = await uploadFile(file);

            if (cropConfig.field === 'poster') {
                setEditingEvent((prev: Event | null) => prev ? ({ ...prev, poster: url }) : null);
            } else if (cropConfig.field === 'film_poster') {
                setEditingFilm((prev: Film | null) => prev ? ({ ...prev, poster: url }) : null);
            } else if (cropConfig.field === 'gallery' && typeof cropConfig.index === 'number') {
                setEditingEvent((prev: Event | null) => {
                    if (!prev) return null;
    ...
    const handleFilmDelete = async (film: Film, rowName: 'row1' | 'row2') => {
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="text-sm text-gray-300 divide-y divide-white/5">
                                    {contactMessages && contactMessages.length > 0 ? (
                                        contactMessages.map((msg, idx: number) => (
